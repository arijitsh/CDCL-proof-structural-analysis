...
def set_hierarchical_tree_style(hierarchical_tree, metric):
    visual_style = {}
    visual_style["layout"] = hierarchical_tree.layout_reingold_tilford(root=[
                                                                       0])
    visual_style["bbox"] = (5000, 5000)
...
def plot_community_structure(vertex_clustering, path, output_directory):
    ...
    igraph.plot(vertex_clustering, output_directory + filename,
                mark_groups=True, **community_structure_style)
    return


def plot_hierarchical_tree(hierarchical_tree, file, metric):
    filename = file + "_" + metric + ".svg"
    hierarchical_tree_style = set_hierarchical_tree_style(
        hierarchical_tree, metric)
...
def update_output_data(
    ...
    mergeability1norm1_data[len(
        path)-1].append(str(pmi.getMergeabilityScore1Norm1()))
    mergeability1norm2_data[len(
        path)-1].append(str(pmi.getMergeabilityScore1Norm2()))
    mergeability2norm1_data[len(
        path)-1].append(str(pmi.getMergeabilityScore1Norm1()))
    mergeability2norm2_data[len(
        path)-1].append(str(pmi.getMergeabilityScore1Norm2()))
    ...
    inter_edges_data[len(
        path)-1].append(str(compute_intercommunity_edges(vertex_clustering)))
    inter_vars_data[len(
        path)-1].append(str(compute_intercommunity_vars(g, vertex_clustering)))
...
def set_hierarchy_tree_color(hierarchical_tree, current_node, percent, metric):
    hierarchical_tree.vs[current_node][metric +
                                       "_color"] = rgba((255, 245, 245), percent, 0.8)
...
def compute_hierarchical_community_structure(
    ...
    set_hierarchy_tree_color(hierarchical_tree, current_node, (g.modularity(
    ...
    if pmi.getPostResolutionClauseWidth() == 0 or math.isnan(pmi.getPostResolutionClauseWidth()):
        ...
        set_hierarchy_tree_color(
            hierarchical_tree, current_node, 0, "mergeability")
        return
    ...
    if False:
        ...
        hierarchical_tree.add_edges(create_edge_list_hierarchical_tree(
            current_node, len(vertex_clustering), hierarchical_tree.vcount()-1))
...
if __name__ == "__main__":
    ...
    # write_data(file, ".modularity", modularity_data)
    ...
    print(f"mergeability1norm1: {mergeability1norm1_data[0][0]}")
    print(f"mergeability1norm2: {mergeability1norm2_data[0][0]}")
    print(f"mergeability2norm1: {mergeability2norm1_data[0][0]}")
